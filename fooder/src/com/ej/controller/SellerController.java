...
//		session.setAttribute("loginUser", user);
	...
	public String my_store(Model model, String u_id) {
		System.out.println("æ»≥Á ≥≠ ºø∑Ø∏ﬁ¿Œ¿Ãæﬂ");
		u_id = "admin66";
	...
	// test?ö©!! seller_store_detail.jsp Î≥¥Ïó¨Ï£ºÍ∏∞
	@RequestMapping("/seller_store_detail_test.ej")
	public String seller_store_detail_test(HttpServletRequest request) {
		// TEST !!!
		Store store = new Store(9090098358.0, "test store", "?Öå?ä§?ä∏", 127.123, 34.123, "devices.jpg", "11:00-22:00", "?Öå?ó§??Î°? 212", 5, "test_id");
	...
	// seller_store_detail.jsp Î≥¥Ïó¨Ï£ºÍ∏∞
...
//		storebiz.modify(store);
	...
	public void seller_store_modify_food(HttpServletRequest request, HttpServletResponse res) {
		...
		if(foods.size() != idx-1) {	// ?†úÍ±∞Îêú food ?ûà?ùÑ Í≤ΩÏö∞
			for(Food f : foods) {
	...
	public void getLatLot(HttpServletResponse res, String addr) {
		...
		String clientId = "MoWWppcCbc3maWhpTijC";//?ï†?îåÎ¶¨Ï??ù¥?Öò ?Å¥?ùº?ù¥?ñ∏?ä∏ ?ïÑ?ù¥?îîÍ∞?";
        String clientSecret = "WHLQ_6RSaM";//?ï†?îåÎ¶¨Ï??ù¥?Öò ?Å¥?ùº?ù¥?ñ∏?ä∏ ?ãú?Å¨Î¶øÍ∞í";
